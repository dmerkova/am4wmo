...
dy=pd.read_csv('w231.csv',names=["sid","level","NA","TB", "TSTD","NR" ],skiprows=2 )
...
with open('wamdar.txt','w') as f:
    sys.stdout = f # Change the standard output to the file we created.
    print('           AMDAR Statistics Versus the NCEP Guess')
    print('                  '+ MONTH + ' ' + YYYY)
    print('Suspect Wind                  ')
...
with open('sus-wamdar.txt','w') as f:
    sys.stdout = f # Change the standard output to the file we created.
    print (dz.sid.to_string(index=False))
